...
def flipOverHor(board):
	return [	board[2],
				board[1],
				board[0]	]
#takes in 3x3 board and reflects it over the line from 2,0 to 0,2
def flipOverUpRight(board):
	return [	[ board[2][2] , board[1][2] , board[0][2] ],
				[ board[2][1] , board[1][1] , board[0][1] ],
				[ board[2][0] , board[1][0] , board[0][0] ]		]
#takes in 3x3 board and reflects it over the line from 0,0 to 2,2
...
def needToBlock(board, toPlay):
	...
	for i in range(3):
		...
		if row.count(opp) == 2 and row.count(toPlay) == 0:
			blockSpots.append( ( i , row.index(0) ) )
			blockReq = True

		col = [ board[0][i] , board[1][i] , board[2][i] ]
		if col.count(opp) == 2 and col.count(toPlay) == 0:
			blockSpots.append( (col.index(0) , i ))
			blockReq = True
	...
	if d0.count(opp) == 2 and d0.count(toPlay) == 0:
		blockSpots.append( ( d0.index(0) , d0.index(0) ) )
		blockReq = True

	if d1.count(opp) == 2 and d1.count(toPlay) == 0:
		blockSpots.append( ( d1.index(0) , int(2- d1.index(0)) ) )
		blockReq = True

	return blockReq, blockSpots

def canWin(board, toPlay):
	if toPlay == 1:
		opp = -1
	elif toPlay == -1:
		opp = 1
	...
	for i in range(3):
		row = board[i]
		if row.count(opp) == 0 and row.count(toPlay) == 2:
			winSpots.append( ( i , row.index(0) ) )
			winPossible = True

		col = [ board[0][i] , board[1][i] , board[2][i] ]
		if col.count(opp) == 0 and col.count(toPlay) == 2:
			winSpots.append( (col.index(0) , i ))
			winPossible = True

	d0 = [ board[0][0], board[1][1], board[2][2] ]
	d1 = [ board[0][2], board[1][1], board[2][0] ]

	if d0.count(opp) == 0 and d0.count(toPlay) == 2:
		winSpots.append( ( d0.index(0) , d0.index(0) ) )
		winPossible = True

	if d1.count(opp) == 0 and d1.count(toPlay) == 2:
		winSpots.append( ( d1.index(0) , int(2- d1.index(0)) ) )
		winPossible = True

	return winPossible, winSpots
...
for play in range(0,10):
	...
	for i in range(len(gamesContinued[play])):
		...
		for spot in possibleSpots:
			...
			if isTrap(board, player)[0] == True:
				dealWithAfter[i] += [ board ]
			elif isGameOver(board)[0] == False:
			...
			for seq in gamesContinued[play][i]:
				if basisIndex in gamesFinished[play+1].keys():
					...
					if newSeq in gamesFinished[play+1][ basisIndex ]:
						...
						gameCount['terminal']['nonDuplicates'][player][play+1] += 1
